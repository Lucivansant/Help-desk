...
if (isset($_POST['email']) && isset($_POST['senha'])) {
    ...
    if (empty($_POST['email']) or empty($_POST['senha'])) {
        header('location: Login_adm.php?Login_vazio=TRUE');
    } else {
        $Session_login_adm = new Login_adm();
        $Return = $Session_login_adm->Get_adm_API($_POST['email'], $_POST['senha']);
        ...
        if (!empty($Return_json_decode)) {
            ...
            header('location: Login_adm.php?Login_invalido=TRUE');
        }
    }
}
?>
    ...
    <section id="sec-log-cad" style="font-family: 'Roboto', sans-serif;">
        <div id="card-login">
            <div id="header-card-login">
                <h6 id="text-header-login">Login (Administrador) - help desk</h6>
            </div>
            <div id="body-card-login">
                <form method="POST" action="./Login_adm.php">
                    <input type="hidden" name="setor" value="adm">
